from src.generators import filter_by_currency_csv_excel, filter_by_currency_json
from src.pandas_func import read_transaction_csv, read_transaction_xls
from src.processing import filter_by_state, sort_by_date
from src.re_collections_random import search_string
...
dict_actions = [
    {1: {"Для обработки выбран JSON-файл.": get_json_data()}},
...
Доступные для фильтровки статусы: EXECUTED, CANCELED, PENDING\n"""
    ...
    if not transactions:
        ...
        for transaction in transactions:
            ...
            if choic == 1:
                print(
                    f"Сумма: {transaction['operationAmount']['amount']} {transaction['operationAmount']['currency']
                    ['code']}\n"
                )
            else:

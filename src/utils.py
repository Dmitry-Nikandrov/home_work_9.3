...
def get_json_data(filename):
    ...
    get_json_data_logger.info(f"Получение динамической ссылки на json-файл {filename}:{path_to_file}")
    try:
        with open(path_to_file, encoding="utf-8") as file:
            data_list = json.load(file)
            get_json_data_logger.info(f"Преобразование в объект python json-файла {filename}")
            if type(data_list) == list:
                get_json_data_logger.info(f"Окончание преобразования json-файла {filename} в объект python")
                return data_list
            else:
                return []
    except Exception:
        get_json_data_logger.error(f"ошибка преобразования json-файла {filename} в объект python")
        return []
